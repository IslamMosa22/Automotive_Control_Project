...
void MDIO_voidSetPinDirection(u8 A_u8PortNum,u8 A_u8PinNum,u8 A_u8PinDirection)
{
   	if(A_u8PortNum<=PORTD && A_u8PinNum<=PIN7 && A_u8PinDirection<=OUTPUT)
   	{
   		switch(A_u8PortNum)
   		{
   		case PORTA:        switch(A_u8PinDirection)
   		                   {
...
{
	if(A_u8PortNum<=PORTD && A_u8PinNum<=PIN7 && A_u8PinValue<=HIGH)
	   	{
	   		switch(A_u8PortNum)
	   		{
	   		case PORTA:        switch(A_u8PinValue)
	   		                   {
...
{
	u8 Local_u8PinValue=7;

	if(A_u8PortNum<=PORTD && A_u8PinNum<=PIN7 )
		   	{
		   		switch(A_u8PortNum)
		   		{
		   		case PORTA:  Local_u8PinValue = GET_BIT(DIO_PINA_REG,A_u8PinNum) ;   break;

		   		case PORTB:  Local_u8PinValue = GET_BIT(DIO_PINB_REG,A_u8PinNum) ;   break;

		   		case PORTC:  Local_u8PinValue = GET_BIT(DIO_PINC_REG,A_u8PinNum) ;   break;

		   		case PORTD:  Local_u8PinValue = GET_BIT(DIO_PIND_REG,A_u8PinNum) ;   break;
...
{
   	if(A_u8PortNum<=PORTD && A_u8PortDirection<=0xff)
    	{
   		      switch(A_u8PortNum)
   				  {
...
void MDIO_voidSetPortValue(u8 A_u8PortNum,u8 A_u8PortValue)
{
	...
	if(A_u8PortNum<=PORTD && A_u8PortValue<=0xff)
	{
